/*
 ...
 Adapted for Teensy 4.1 by Maxim Bortnikov
 For more information please visit
 https://sourceforge.net/projects/teensy-4-1-tetris/
 https://github.com/Northstrix/Teensy-4.1-Tetris
...
void setup() {
  ...
  trng_init();
  tft.init();
  tft.setRotation(2); //This is the display in portrait
...
bool isButtonPressed(int pin, unsigned long &lastPress) {
  if (digitalRead(pin) == LOW) {
    if (millis() - lastPress >= BUTTON_PRESS_THRESHOLD) {
      lastPress = millis();
      return true;
    }
  }
...
void gameInput() {
  ...
  // Left button
  if (isButtonPressed(LEFT_BUTTON_PIN, lastLeftPress)) {
    if (leftButtonReleased || (currentTime - lastLeftMove >= REPEAT_MOVE)) {
      moveLeft = true;
  ...
  // Right button
  if (isButtonPressed(RIGHT_BUTTON_PIN, lastRightPress)) {
    if (rightButtonReleased || (currentTime - lastRightMove >= REPEAT_MOVE)) {
      moveRight = true;
  ...
  if (isButtonPressed(DROP_BUTTON_PIN, lastDownPress))
    dropDown = true;
  
  if (isButtonPressed(FLIP_BUTTON_PIN, lastDownPress) && !flipButtonPressed) {
    rotatePiece = true;
    flipButtonPressed = true;
  }
  
  if (digitalRead(FLIP_BUTTON_PIN) == HIGH)
    flipButtonPressed = false;
  
  if (isButtonPressed(START_BUTTON_PIN, lastDownPress) && !startButtonPressed) {
    isPaused = !isPaused;
    if (!isPaused) {
...
void show_lock_screen(){
  ...
  if (init_scr == 0 || init_scr == 1){
    disp_centered_text_b_w("Teensy 4.1", 10, 0x0882, 0xf7de);
    disp_centered_text_b_w("Tetris", 35, 0x0882, 0xf7de);
  }
  else{
    disp_centered_text_b_w("Teensy 4.1", 10, 0x1376, 0xf7de);
    disp_centered_text_b_w("Tetris", 35, 0x1376, 0xf7de);
  }
